...
class transaction extends uvm_sequence_item;
  `uvm_object_utils_begin(transaction)
    ...
    `uvm_field_int(rx_start, UVM_DEFAULT)
    ...
    `uvm_field_int(tx_err, UVM_DEFAULT)
...
class mon extends uvm_monitor;
  ...
  virtual task run_phase(uvm_phase phase);
    forever begin
      ...
      if(vif.rst) begin
      ...
      else if(vif.tx_start == 1'b1 || vif.rx_start == 1'b1) begin
        tr = transaction::type_id::create("tr");
        tr.rst         = 1'b0;
        ...
        `uvm_info("MON", $sformatf("BAUD:%0d LEN:%0d PAR_T:%0d PAR_EN:%0d STOP:%0d TX_DATA:%0d RX_DATA:%0d RX_ERR:%0d", tr.baud, tr.length, tr.parity_type, tr.parity_en, tr.stop2, tr.tx_data, tr.rx_out, tr.rx_err), UVM_NONE);
        send.write(tr);
      end
    end
  endtask
...
class sco extends uvm_scoreboard;
  ...
  virtual function void write(transaction tr);
    `uvm_info("SCO", $sformatf("BAUD:%0d LEN:%0d PAR_T:%0d PAR_EN:%0d STOP:%0d TX_DATA:%0d RX_DATA:%0d RX_ERR:%0d", tr.baud, tr.length, tr.parity_type, tr.parity_en, tr.stop2, tr.tx_data, tr.rx_out, tr.rx_err), UVM_NONE);
    if(tr.rst == 1'b1)
    `uvm_info("SCO", "System Reset", UVM_NONE)
    else if(tr.tx_data == tr.rx_out && tr.rx_err == 1'b0)
    `uvm_info("SCO", "Test Passed", UVM_NONE)
    else if(tr.tx_data == tr.rx_out && tr.rx_err == 1'b1)
    `uvm_error("SCO", "Test Failed - Parity/Framing Error Detected")
    else
    `uvm_error("SCO", "Test Failed - Data Mismatch")
    $display("----------------------------------------------------------------");
  endfunction
...
class agent extends uvm_agent;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(uart_config)::get(this, "", "cfg", cfg)) begin
      `uvm_info("AGENT", "Using default agent configuration", UVM_LOW)
...
class test extends uvm_test;
  ...
  virtual task run_phase(uvm_phase phase);
    ...
    `uvm_info("TEST", "Starting rand_baud sequence", UVM_LOW)
    rb.start(e.a.seqr);
    `uvm_info("TEST", "Starting rand_baud_with_stop sequence", UVM_LOW)
    rbs.start(e.a.seqr);
    `uvm_info("TEST", "Starting len5 with parity sequence", UVM_LOW)
    rb5l.start(e.a.seqr);
    `uvm_info("TEST", "Starting len6 with parity sequence", UVM_LOW)
    rb6l.start(e.a.seqr);
    `uvm_info("TEST", "Starting len7 with parity sequence", UVM_LOW)
    rb7l.start(e.a.seqr);
    `uvm_info("TEST", "Starting len8 with parity sequence", UVM_LOW)
    rb8l.start(e.a.seqr);
    `uvm_info("TEST", "Starting len5 without parity sequence", UVM_LOW)
    rb5lwop.start(e.a.seqr);
    `uvm_info("TEST", "Starting len6 without parity sequence", UVM_LOW)
    rb6lwop.start(e.a.seqr);
    `uvm_info("TEST", "Starting len7 without parity sequence", UVM_LOW)
    rb7lwop.start(e.a.seqr);
    `uvm_info("TEST", "Starting len8 without parity sequence", UVM_LOW)
